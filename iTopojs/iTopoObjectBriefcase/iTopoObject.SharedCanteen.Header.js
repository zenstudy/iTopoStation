...
iTopoObjectSharedCanteenHeader.prototype = {
	...
	onClickOutlook: function() {// this对应一个item
		...
		editor.stationDB.fetchiTopoBaseOutlook(scope.taskObject.baseUUID,function(outlookData){
			...
			editor.resourceTracker.loadOutlook('iTopoType/TaskObject/SharedCanteen', function(background_outlook){

				editor.resourceTracker.loadModel("iTopoType/TaskObject/SharedCanteen", originPosition,iTopoEarthSettings.standMaxBoxW/3, function(baseModel){
	...
	onTaskCardsClassCSS3D: function() {
		var scope = this;
		var title = editor.strings.getKey( 'sidebar/SharedCanteen/Header/iTopoTaskCards' ) ;
		var originPosition = new THREE.Vector3();
