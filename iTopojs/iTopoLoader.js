...
import { iTopoLoaderUtils } from './iTopoLoaderUtils.js';

import { JSZip } from '../../examples/jsm/libs/jszip.module.min.js';

function iTopoLoader( editor ) {
	...
	this.loadItemList = function ( items ) {
		...
		iTopoLoaderUtils.getFilesFromItemList( items, function ( files, filesMap ) {
	...
	this.loadFiles = function ( files, filesMap ) {
		...
		if ( files.length > 0 ) {
			...
			var filesMap = filesMap || iTopoLoaderUtils.createFilesMap( files );
	...
	function isGLTF1( contents ) {
		...
		if ( typeof contents === 'string' ) {
			...
			var magic = THREE.iTopoLoaderUtils.decodeText( new Uint8Array( contents, 0, 4 ) );
			...
			if ( magic === 'glTF' ) {
				...
				resultContent = THREE.iTopoLoaderUtils.decodeText( new Uint8Array( contents ) );
